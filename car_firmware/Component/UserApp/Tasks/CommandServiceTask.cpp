...
void CommandServiceTask::process_command(etl::string_view frame)
{
    ...
    if (name == "control") {
        if (args == "off") {
            ...
            if (status_.state() != SystemState::ready || control_.storage_blocks_control()) {
                ...
                uart.print("control: on; waiting for normal startup conditions\n");
            }
        } else uart.print("control: usage: control off|on\n");
        return;
    }
    if (name == "status") {
    ...
    if (args.empty()) {
        const PidGains* gains = name == "anglepid" ? &parameters.angle :
            ...
            (name == "rollpid" || name == "legpid") ? &parameters.roll : nullptr;
        if (gains != nullptr) {
    ...
    if (name == "showimu" || name == "showrpm") {
        ...
        if (args == "-auto") {
            ...
            accepted = true;
        } else if (text_command::ParsedCommand option;
                   text_command::parse(args, option) && option.command == "-manual") {
            accepted = parse_value(option.args, parameters.angle.kp);
            if (accepted) parameters.angle_kp_auto = false;
        } else {
    ...
    } else if (name == "rollpid" || name == "legpid") {
        ...
        if (accepted) {
            const float height = BalanceCompensation::clampLegHeight(parameters.leg_height);
            float result_x{};
...
void CommandServiceTask::save_parameters(etl::string_view args)
{
    auto& uart = board_.command_uart();
    const auto mode = MotionSettings::parseSaveMode({args.data(), args.size()});
    if (mode == MotionSettings::SaveMode::invalid) {
        uart.print("save: usage: save [all|recycle]\n");
        return;
    }
    if (status_.state() == SystemState::booting) {
        uart.print("save: busy; system is starting\n");
        return;
    }
    switch (persistence_.save(mode == MotionSettings::SaveMode::recycle)) {
    case MotionSettings::SaveResult::saved: uart.print("save: ok (all motion parameters)\n"); break;
    case MotionSettings::SaveResult::unchanged: uart.print("save: unchanged (no flash write)\n"); break;
...
{
    auto& uart = board_.command_uart();
    const auto p = control_.parameters();
    const auto f = control_.feedback();
    const auto legs = control_.leg_targets();
    ...
    uart.print("legheight={:.4f} target_roll={:.4f} left={:.4f} right={:.4f}\n",
        p.leg_height, p.roll_target, legs.left, legs.right);
}
