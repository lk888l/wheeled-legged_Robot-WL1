...
更换 MPU6050 或机械安装后应重新标定。`anglebias` 现在设置双腿均为
`44.5 mm` 时的重心俯仰基准，默认 `9.5°`。在支架上从最低腿高开始小步调整，
例如发送 `anglebias 10.5`；可直接发送到车端 USART1，也可发送到当前遥控器的
USART1 串口桥接（以 LF 结束，不加 `nrfsend` 前缀）。

控制代码按限幅后双腿目标的平均高度叠加补偿，因此升高腿部后实际反馈偏置会变化，
但基准不会被后续 `R` 或 `legheight` 命令覆盖。基准只保存在 RAM 中，复位恢复
默认值；确认设置后再修改 `CtrlAlgorithm/BalanceCompensation.hpp` 中的
`default_minimum_bias_degrees` 并重新构建。仍需检查不同
腿高和横滚状态下的控制响应，不能把串口或无线 ACK 当作标定通过。
