...
| 参数 | 默认值 | 说明 |
| --- | ---: | --- |
| Angle `Kp / Ki / Kd` | `75.35 / 0 / 60` | `-p` 调整 61.5 mm 基准并启用高度补偿；`-manual` 设置固定 Kp |
...
MotionControl 的休眠、I2C、VQF、PID 和日志均不在临界区内。临界区保护
`ControlState` 短值复制、`RuntimeStatus` 状态变更，以及与保存互锁原子提交的
有限次 PWM/GPIO 寄存器写入；Flash 编程和擦除在临界区外执行。按键事件队列采用单生产者/
...
- 控制参数修改先在 RAM 生效，`save` 后复位恢复保存值；无有效记录时使用编译默认值。
...
保存中互锁禁止解锁；即使保存全部发生在两个控制周期之间，也会重置启动门控。
最终电机输出与反馈发布原子检查互锁，防止命令恰好抢占解锁周期导致误输出。
