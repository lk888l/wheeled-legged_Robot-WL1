...
int main()
{
    ...
    CHECK(harness.control.parameters().angle_bias == 9.5F);
    ...
    harness.board.imu().reading = {0.0, -9.5, 0.0};
    ...
    CHECK(harness.board.wheel_motor().writes == 140U);
    ...
    for (const auto& sample : harness.samples) {
        const float baseline = sample.tick < 20U ? 9.5F : sample.tick < 30U ? 10.5F
        ...
        const int pwm = static_cast<int>(std::round(kp * (9.5 - bias)));
        CHECK(sample.left_pwm == pwm && sample.right_pwm == pwm);
    }

    // Startup uses the real minimum height, before the first 50 ms servo update.
    CHECK(harness.at(10U).feedback.angle_bias == 9.5F);
    CHECK(harness.at(10U).left_pwm == 0);
    CHECK(harness.at(20U).left_pwm == -70);
    CHECK(harness.at(30U).left_pwm == -141);
    ...
    CHECK(harness.at(60U).left_pwm != harness.at(50U).left_pwm);
    ...
    CHECK(positive.legs.left == 44.5F && positive.legs.right > 50.0F);
    CHECK(positive.legs.right < 51.0F);
    CHECK(positive.legs.left == negative.legs.right);
    CHECK(positive.legs.right == negative.legs.left);
    ...
    for (TickType_t tick = 350U; tick <= 700U; tick += 10U) {
        ...
        CHECK(harness.at(tick).left_pwm == harness.at(60U).left_pwm);
    }
}
