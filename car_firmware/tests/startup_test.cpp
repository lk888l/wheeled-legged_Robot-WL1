...
int main(int argc, char** argv)
{
    ...
    size_t line = 0U;
    CHECK(logs.size() == 3U + initialized_modules + expected_tasks.size() + (skip_command ? 1U : 0U) + (radio_enabled ? 0U : 1U));
    ...
    for (size_t i = 0U; i < initialized_modules; ++i) {
        ...
        if (i == 0U) { CHECK(logs[line++] == "params: compiled defaults (no valid flash record)\n"); }
    }
