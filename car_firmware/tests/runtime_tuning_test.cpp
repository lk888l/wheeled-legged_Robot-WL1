...
void test_calibration_and_recovery()
{
    ...
    step = [&] {
        ...
        if (tick == 720U) {
            CHECK(std::fabs(f.control.feedback().angle_kp - 80.0F) < 0.0001F);
...
void test_save_at_arming_boundary()
{
    ...
    Fixture f;
    bool saved = false;
    // At sample 50 the local gate has armed, but the shared feedback still
    // describes sample 49. Simulate the command task preempting encoder I/O.
    ...
    step = [&] {
        if (fake_rtos::now == 510U) {
            CHECK(saved && !f.control.storage_busy());
            CHECK(f.board.wheel_motor().writes == 0U);
        }
        if (fake_rtos::now <= 1000U) {
            CHECK(!f.control.feedback().armed);
            CHECK(f.board.wheel_motor().left == 0 && f.board.wheel_motor().right == 0);
        }
        if (fake_rtos::now == 1010U) {
            CHECK(f.control.feedback().armed);
            CHECK(f.status.control_enabled());
            f.send("control off", false);
        }
        if (fake_rtos::now == 1020U) {
            CHECK(!f.control.feedback().armed);
            CHECK(f.board.wheel_motor().left == 0 && f.board.wheel_motor().right == 0);
            f.send("save", false);
            CHECK(!f.status.control_enabled());
        ...
        if (fake_rtos::now > 1020U && fake_rtos::now <= 1510U) {
            CHECK(!f.control.feedback().armed);
        }
        if (fake_rtos::now == 1520U) {
            CHECK(f.control.feedback().armed);
            // Both commands arrive between two motion ticks.
            f.send("control off", false);
            f.send("control on", false);
        }
        if (fake_rtos::now > 1520U && fake_rtos::now <= 2010U) {
            CHECK(!f.control.feedback().armed);
        }
        if (fake_rtos::now == 2020U) {
            CHECK(f.control.feedback().armed);
            throw fake_rtos::LoopDone{};
        }
    };
